...
class PixelCNN(distribution.Distribution):
  ...
  def _make_mixture_dist(self, component_logits, locs, scales, join_events=True):
    ...
    logistic_dist = quantized_distribution.QuantizedDistribution(
          distribution=transformed_distribution.TransformedDistribution(
              distribution=transformed_distribution.TransformedDistribution(
                  distribution=logistic.Logistic(loc=locs, scale=scales), bijector=scale.Scale(self._high-self._low)),
              bijector=shift.Shift(self._low - 0.5)
          ),
          low=self._low - 1,
          high=self._high + 1
      )
...
#     return independent.Independent(dist, reinterpreted_batch_ndims=2)
  ...
  def _log_prob(self, value, conditional_input=None, training=None, join_events=True):
    ...
    dist = self._make_mixture_dist(component_logits, locs, scales, join_events=join_events)
    if join_events:
